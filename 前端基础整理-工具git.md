...
进程是具有一定独立功能的程序，它是系统进程资源分配和调度的一个独立单元，并且进程都有自己的地址空间。

##### 线程

线程是进程的一个实体，是CPU调度和分派的基本单元。
...
4.进程有独立的地址空间，一个进程崩溃后，在保护模式下不会对其它进程产生影响，而线程只是一进程中的不同执行路径但线程之间没有单独的地址空间，一个线程死掉就等于整个进程死掉，所以多进程的程序要比多线程的程序健壮，但在进程切换时，耗费资源较大，效率要差一些。
...
词法作用域也就是在词法阶段定义的作用域。
换句话说，词法作用域就是你在写代码的时候就已经决定了`变量的作用域。`

> js中其实只有词法作用域，并没有动态作用域，this的执行机制让作用域表现的像动态作用域，this的绑定是在代码执行的时候确定的。

https://www.cnblogs.com/xiaohuochai/p/5700095.html  `词法作用域解析--网络上还有典例`
